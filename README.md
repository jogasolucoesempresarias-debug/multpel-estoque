...
- **Transferência entre filiais NÃO é compra** (decisão do diretor 07/2026): pedido cujo **fornecedor é a própria empresa** (mesma **raiz de CNPJ**, 8 dígitos, contra `MULTPEL_EMPRESA["cnpj"]`) fica **fora do orçamento** — é o CD abastecendo as lojas. Comparo pela raiz e não pelo `CODFORNEC` porque o código muda e a raiz pega qualquer filial. Impacto medido: comprado da empresa **−R$ 164.980,42** no mês, e a compradora dessas transferências (que só tinha esses pedidos) zera.
  - O resumo devolve `transf_n`/`transf_valor` e a tela **avisa quantos pedidos foram excluídos e quanto** — sem isso o valor cai sem explicação e vira a próxima desconfiança. Foi assim que o assunto apareceu: o card mostrava R$ 164.980,42 "comprado" com "nenhum pedido em aberto", o que parecia duplicação (não era — os pedidos eram reais e distintos, só estavam 100% entregues).
  - ⚠️ **Escopo**: só o orçamento. O `ja_pedida` (pedido em aberto que desconta da sugestão de compra) **continua contando** a transferência — mercadoria a caminho é mercadoria a caminho. Mudar isso alteraria a sugestão de compra e não foi pedido.
- **Venda/lucro/margem** = `FATURAMENTO_VENDAS` (RCA), líquida (− devoluções), por período.
- **Nomenclatura ABC/XYZ fica como é** (questionada pelo diretor 07/2026, decisão de manter): XYZ é padrão de mercado (previsibilidade da demanda, dupla da curva ABC), e como o produto será vendido a grandes atacadistas, o termo padrão é reconhecido pelo supply chain do cliente — nome próprio faria parecer que reinventamos algo que já existe. O acordo foi **manter a sigla sempre colada ao significado**: a matriz já trazia "X estável / Y variável / Z errático" + estratégia por zona, e o **filtro do topo** passou a mostrar `X - Estável` / `Y - Variável` / `Z - Errático`.
  - ⚠️ As `<option>` do `#f-xyz` **precisam de `value` explícito** (`X`/`Y`/`Z`). Sem ele o rótulo vira o valor, `p.xyz !== f.xyz` nunca casa e o filtro retorna **zero produtos em silêncio** (comprovado: filtrando pelo rótulo a API devolve 0 de 2.909). O mesmo `value` é escrito de volta pelo clique na matriz ABC-XYZ (`k[1]`) e viaja p/ o servidor em `filtrosQS`/`exportQS`.
